...
def test_calc_data_using_namedtuple_len():
    result = calc_data_using_namedtuple()
    assert len(result) == 4, "Not giving the required output data_using_named_tuple"

def test_calc_data_using_namedtuple_parametr():
    result = calc_data_using_namedtuple()
    assert result._fields == ('largest_blood_type', 'mean_current_location', 'oldest_person', 'average_age'), "fields are not correct in named tuples"
...
def test_calc_data_using_dictionary_len():
    result = calc_data_using_dictionary()
    assert len(result.keys()) == 4, "Not giving the required op for data_using_dictionary"

def test_calc_data_using_dictionary_parametr():
    result = calc_data_using_dictionary()
    assert list(result) == ['largest_blood_type', 'mean_current_location', 'oldest_person', 'average_age'], "fields are not correct in named tuples"

def test_calc_data_using_dictionary_type():
    result = calc_data_using_dictionary()
